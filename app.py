...
APP_TITLE = "Curvas de nivel COP30 para quebradas ≤ 2.000 km²"
...
RECOMENDACIONES_QUEBRADAS_2000 = """
Parámetros recomendados para quebradas y cuencas ≤ 2.000 km²:
...
- Equidistancia de curvas: ajustable desde 1 m
- Para uso estable: 10 m, 20 m o 25 m
- Para máximo detalle: 1 m a 5 m, solo en áreas pequeñas o con resolución interna/remuestreo adecuado
- Simplificación: 10 m a 30 m
- Evitar equidistancias de 1 m a 5 m en áreas cercanas a 2.000 km², porque generan KMZ muy pesados.
"""
...
st.title(APP_TITLE)
st.caption("Aplicación para descargar DEM COP30 desde OpenTopography, procesar quebradas o cuencas de hasta 2.000 km² y exportar curvas de nivel ajustables desde 1 m en un KMZ unificado.")

with st.expander("Criterio técnico importante", expanded=True):
    st.write(
        "Esta versión está ajustada para quebradas y cuencas con superficie menor o igual a 2.000 km². "
        "Para este rango no conviene usar parámetros de cuencas gigantes. La aplicación mantiene división por DEM parciales, "
        "pero con valores más detallados y estables para trabajar con COP30 en Streamlit Cloud."
    )
    st.info(RECOMENDACIONES_QUEBRADAS_2000)
...
with col_a:
    ...
    area_obj = st.number_input("Área objetivo aproximada km²", min_value=10.0, max_value=2000.0, value=500.0, step=50.0)
    radius_km = st.number_input("Radio desde el punto km", min_value=2.0, max_value=50.0, value=15.0, step=1.0)
    buffer_km = st.number_input("Buffer adicional para polígono km", min_value=0.0, max_value=20.0, value=2.0, step=1.0)

    st.subheader("3. División y curvas")
    n_tiles = st.number_input("Número de DEM parciales", min_value=1, max_value=12, value=4, step=1)
    interval_m = st.number_input("Equidistancia curvas de nivel m", min_value=1.0, max_value=200.0, value=25.0, step=1.0, help="Puede comenzar en 1 m. Para áreas grandes, usar 1 a 5 m puede generar miles de curvas y un KMZ muy pesado.")
    target_resolution_m = st.selectbox("Resolución interna para procesar curvas", [30, 60, 90, 150], index=1)
    simplify_m = st.number_input("Suavizado/simplificación de líneas m", min_value=0.0, max_value=200.0, value=20.0, step=5.0)
    max_levels = st.number_input("Máximo de cotas por tile", min_value=20, max_value=5000, value=500, step=50, help="Protección para no saturar Streamlit. Si la equidistancia es muy baja, aumenta este valor solo para áreas pequeñas.")
    max_vertices_per_line = st.number_input("Máximo vértices por curva", min_value=50, max_value=15000, value=3000, step=100)
    max_total_lines = st.number_input("Máximo total de curvas/segmentos", min_value=1000, max_value=200000, value=100000, step=1000)

    with st.expander("Parámetros recomendados para quebradas ≤ 2.000 km²", expanded=False):
        st.markdown(RECOMENDACIONES_QUEBRADAS_2000)

    st.subheader("4. Bbox manual")
    man_s = st.number_input("south", value=-31.00, format="%.8f")
    man_n = st.number_input("north", value=-30.75, format="%.8f")
    man_w = st.number_input("west", value=-71.15, format="%.8f")
    man_e = st.number_input("east", value=-70.90, format="%.8f")
...
with col_b:
    ...
    if bbox is None:
        ...
        if area_est > 2_500:
            st.error("El área de trabajo excede el criterio de esta versión. Reduce bbox/radio/buffer para trabajar con quebradas ≤ 2.000 km².")
        elif area_est > 2_000:
            st.warning("El bbox supera 2.000 km². Puede ser aceptable solo si el polígono real de cuenca es menor, pero conviene reducir el área de descarga.")
        if target_resolution_m < rec_res:
            st.warning(f"Para esta superficie se recomienda resolución interna aproximada ≥ {rec_res} m si la app se pone lenta.")
        if interval_m <= 5:
            if area_est > 300:
                st.warning("Equidistancia muy detallada. Para áreas sobre 300 km², curvas de 1 a 5 m pueden generar KMZ muy pesado o proceso lento. Recomendado: aumentar DEM parciales, subir resolución interna a 60/90 m o usar curvas ≥ 10 m.")
            else:
                st.info("Equidistancia detallada activada. Curvas de 1 a 5 m son más adecuadas para áreas pequeñas o análisis local.")
        if est_processed_cells_per_tile > 3_000_000:
            st.error("Cada tile sigue siendo pesado para Streamlit Cloud. Aumenta número de DEM parciales o sube resolución interna a 60/90/150 m.")
        elif est_processed_cells_per_tile > 1_500_000:
            st.warning("Carga media/alta. Puede funcionar, pero conviene usar curvas cada 20/25 m y simplificación ≥ 20 m.")
        else:
            st.success("Configuración razonable para quebradas ≤ 2.000 km².")
...
if run:
    ...
    if not (south < north and west < east):
        ...
        st.stop()
    area_run = bbox_area_km2(south, north, west, east)
    if area_run > 2_500:
        st.error(f"Área de trabajo demasiado grande para esta versión ({area_run:,.0f} km²). Reduce el bbox/radio/buffer. Límite operativo: alrededor de 2.000 km².")
        st.stop()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        kmz_path = tmpdir / "curvas_quebrada_2000km2_unificado.kmz"
        summary_path = tmpdir / "resumen_curvas_quebrada_2000km2.json"

        with open(kml_path, "w", encoding="utf-8") as kml:
            kml.write(kml_header("Curvas de nivel unificadas - quebrada ≤ 2.000 km²"))
        ...
        summary = {
            ...
            "criterio_version": "quebradas_y_cuencas_hasta_2000_km2",
        ...
        with open(kmz_path, "rb") as f:
            st.download_button("Descargar KMZ unificado", f, file_name="curvas_quebrada_2000km2_unificado.kmz", mime="application/vnd.google-earth.kmz")
        with open(summary_path, "rb") as f:
            st.download_button("Descargar resumen JSON", f, file_name="resumen_curvas_quebrada_2000km2.json", mime="application/json")
...
st.divider()
st.markdown(
    "**Nota:** esta versión está calibrada para quebradas y cuencas hasta aproximadamente 2.000 km². "
    "El KMZ unificado contiene todos los segmentos generados por tile en un solo archivo. "
    "Si se requiere unión topológica perfecta entre bordes de tiles, puede realizarse un postproceso GIS local; "
